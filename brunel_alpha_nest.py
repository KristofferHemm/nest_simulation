...
###############################################################################
# First, define the `Lambert W` function implemented in SLI. The second function 
# computes the maximum ofvthe postsynaptic potential for a synaptic input 
# current of unit amplitude (1 pA) using the `Lambert W` function. Thus function 
# will later be used to calibrate the synaptic weights.
...
tauSyn = new_tauSyn  # synaptic time constant in ms
tauMem = new_tauMem  # time constant of membrane potential in ms
theta = new_theta  # membrane threshold potential in mV
...
J = 0.10       # postsynaptic amplitude in mV
...
espikes = nest.Create("spike_recorder", params={'record_to': 'memory'})
ispikes = nest.Create("spike_recorder", params={'record_to': 'memory'})
...
print(f'tauSyn: {new_tauSyn}')
print(f'tauMem: {new_tauMem}')
...
combined_meta = {meta_key.encode(): meta_json.encode()}
table = table.replace_schema_metadata(combined_meta)

pq.write_table(table, f'ban_{new_tauSyn}_{new_tauMem}_{new_theta}_{new_seed}.parquet')
